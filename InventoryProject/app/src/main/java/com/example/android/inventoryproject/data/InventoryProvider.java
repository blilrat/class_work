...
public class InventoryProvider extends ContentProvider {
    ...
    static {
        ...
        // The content URI of the form "content://com.example.android.inventoryproject/inventory" will map to the
    ...
    private Uri insertInventory(Uri uri, ContentValues values) {
        ...
        // If the qty is provided, check that it's greater than or equal to 0
        Integer quantity = values.getAsInteger(InventoryEntry.COLUMN_QTY_AVAILABLE);
    ...
    private int updateInventory(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If the {@link InventoryEntry#COLUMN_QTY_AVAILABLE} key is present,
        if (values.containsKey(InventoryEntry.COLUMN_QTY_AVAILABLE)) {
            // If the qty is provided, check that it's greater than or equal to 0
            Integer quantity = values.getAsInteger(InventoryEntry.COLUMN_QTY_AVAILABLE);
            if (quantity != null && quantity < 0) {
                throw new IllegalArgumentException("Item quantity must be non-negative.");
